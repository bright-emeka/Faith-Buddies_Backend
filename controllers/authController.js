...
export const login = async (req, res, next) => {
  try {
    ...
    if (user.authProvider === 'firebase') {
      return res.status(401).json({ error: 'Please use Firebase authentication or set a native password' });
    }

    if (!user.passwordHash) {
      return res.status(401).json({ error: 'No password set. Please use Firebase authentication or reset your password.' });
    }
